...
- Список карточек товаров. В каждой карточке указывать: название, размер, список цветов, количество цветов
...
- Использовать `localStorage` для сохранения текущего списка товаров при закрытии вкладки

## Реализация

`model.js` содержит класс `ClothingItem` с приватными полями и методами для управления цветами. Метод `toJSON()` используется для сохранения в `localStorage`.
 
`main.js` при загрузке страницы восстанавливает товары из `localStorage` и отрисовывает список карточек. Добавление и удаление товаров и цветов выполняется асинхронно через Promise и `setTimeout`. После каждого изменения список перерисовывается заново.
